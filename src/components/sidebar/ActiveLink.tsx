...
const ActiveLink = ({ to, label, Icon, onClick }: Props) => {
    return (
        <NavLink
            ...
            className={({ isActive }) => {
                const base = `
                    relative flex items-center gap-2 text-[18px] font-medium px-3 mt-2
                    transition-all duration-300 ease-out group
                `;

                const activeStyle = `text-main! dark:text-white font-semibold before:h-full`;
                ...
                return `
                    ...
                    before:absolute before:left-0 before:top-0 before:h-0 before:w-[3px]
